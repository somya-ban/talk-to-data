...
class SchemaGraphBuilder:
    ...
    def add_metric_node(self, _metric_name: str, _columns: list, _table_name: str):
        """
        Interface method called by app.py after metric confirmation.
        Metric nodes are optional in the Neo4j graph — Table nodes and
        their relationships are sufficient for join path discovery.
        """
        pass
    ...
    # ── Count methods (used by app.py init response) ───────────────────────────

    def node_count(self) -> int:
        """Return count of Table nodes in graph."""
        with self.driver.session() as session:
            return session.execute_read(
                lambda tx: tx.run("MATCH (t:Table) RETURN COUNT(t) AS c").single()["c"]
            )
    ...
    def edge_count(self) -> int:
        """Return count of relationship edges in graph."""
        with self.driver.session() as session:
            return session.execute_read(
                lambda tx: tx.run("MATCH ()-[r]->() RETURN COUNT(r) AS c").single()["c"]
            )
