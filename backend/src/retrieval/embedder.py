...
class SchemaEmbedder:
    ...
    def store_qa_pair(
        self, question: str, sql: str, was_corrected: bool = False
    ) -> str:
        """Store a Q-SQL pair with correction metadata."""
        doc_id = self._make_id(question)
        self._upsert(
            self.sql_col,
            doc_id,
            question,
            metadata={"sql": sql, "was_corrected": str(was_corrected)},
        )
        return doc_id

    def qa_count(self) -> int:
        """Alias for sql_count — used by app.py status and init responses."""
